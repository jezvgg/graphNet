...
class FileDialogCore:
    def __init__(self, config: FileDialogConfig, state: FileDialogState):
        self.config = config
        self.state = state or FileDialogState()
        ...
        #REMOVE
        self._setup_compatibility_adapter()
    
    def _setup_compatibility_adapter(self):
        """Временный адаптер для обратной совместимости со старым UI."""
        # Перенаправляем обращения к старым полям
        self.state.file_list_cache = self.state._file_list_cache
        # ... другие поля по мере необходимости
        
        # Регистрируем обработчик для синхронизации кэша
        def on_selection_changed(event, payload):
            if event == "selection_changed":
                # Обновляем кэш выделения для старого UI
                for item in self.state.file_list_cache:
                    item["_selected"] = item["path"] in self.state.selected_files
        
        self.state.add_observer(on_selection_changed)
    ...
    def refresh_directory(self):
        """Обновить содержимое текущей директории — используем новый сканер."""
        try:
            current_path = Path(self.state.current_path)
            if not current_path.exists() or not current_path.is_dir():
                raise FileNotFoundError(f"Directory not found: {current_path}")

            # ЕДИНСТВЕННОЕ ИЗМЕНЕНИЕ: заменяем старую логику на сканер
            file_items = self.scanner.scan_directory(
                current_path,
                search_query=self.state.search_query,
                file_filter=self.state._filter_selected
            )

            # Преобразуем FileItem → dict для старого UI (остаётся совместимость!)
            file_infos = [{
                "name": item.name,
                "path": item.path,
                "is_dir": item.is_dir,
            ...
            self._update_table(file_infos)  # ← без изменений!
    ...
    def _on_file_click(self, sender, app_data):
        """Обработчик клика — теперь использует метод состояния."""
        import dearpygui.dearpygui as dpg
        
        # Получаем данные из UI (остаётся совместимость)
        user_data = dpg.get_item_user_data(sender)
        clicked_path = user_data["path"]
        clicked_index = user_data["index"]
        
        # Состояние само определит логику выделения
    ...
    def _on_file_double_click(self, sender, app_data):
        """Обработчик двойного клика — переносим логику в состояние."""
        import dearpygui.dearpygui as dpg
        user_data = dpg.get_item_user_data(sender)
        clicked_path = user_data['path']
        ...
        if path.is_dir():
            self.state.current_path = str(path)  # ← уведомит наблюдателей!
        else:
            # Для файлов — пока оставляем старую логику
